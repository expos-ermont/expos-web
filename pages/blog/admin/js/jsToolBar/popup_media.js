...
{var player=$('#public_player').val();var align=$('input[@name="alignment"]:checked',insert_form).val();var title=insert_form.elements.title.value;if(title){player=player.replace(/(name="FlashVars"\s+value=")/,'$1title='+encodeURI(title)+'&amp;');}
$('object',player).attr('width',$('#video_w').val());$('object',player).attr('height',$('#video_h').val());if(align!=undefined&&align!='none'){player='<div style="'+media_align_grid[align]+'">'+player+'</div>';}
