...
if (!is_file(DC_RC_PATH))
{
	...
	__error('Unable to open config file'
		,'There doesn\'t seem to be a <strong>config.php</strong> file. '.
		'I need this before we can get started. Need more help? '.
		'<a href="http://doc.dotclear.net/2.0/admin/install">We got it</a>. '.
		'You can <a href="'.$path.'install">create a <strong>config.php</strong> '.
		'file through a web interface</a>, but this doesn\'t work for all '.
...
try {
	...
	__error($e->getMessage()
		...
		'the database server at "<em>'.DC_DBHOST.'</em>". This could mean your '.
