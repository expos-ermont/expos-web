...
class dcImportWP extends dcIeModule
{
	...
	public function gui()
	{
		...
		switch ($this->step)
		{
			...
			case 4:
				...
				break;
			case 5:
				$t = sprintf(__('Importing entries from %d to %d'),$this->post_offset,$this->post_offset+$this->post_limit);
	...
	protected function importPosts(&$percent)
	{
		...
		if ($this->post_offset > $count) {
			$percent = 100;
		} else {
			$percent = $this->post_offset * 100 / $count;
		}
	}
	...
	protected function importPost(&$rs,&$db)
	{
		...
		# Create tags if we have metadata plugin in DC2
		if (class_exists('dcMeta')) {
			...
			if (!$old_cat_ids->isEmpty() && $this->vars['cat_as_tags']) 
			{
				...
				while ($old_cat_ids->fetch()) {
					$meta->setPostMeta($cur->post_id,'tag',$this->cleanStr($this->vars['cat_tags_prefix'].$old_cat_ids->name));
				}
			}
		}
	}
